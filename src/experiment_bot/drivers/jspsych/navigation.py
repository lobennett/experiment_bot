...
async def navigate_page(page: Page) -> dict:
    ...
    # 2c. Trial-body plugin (keyboard-response, poldracklab-stop-signal,
    #     etc.) whose hook isn't yet armed (between plugin start and the
    #     getKeyboardResponse call). Wait quietly. Do NOT dispatch keys —
    #     any keystroke at the display root during this window can be
    #     captured as the response for whichever listener arms next.
    if "keyboard-response" in type_name or "stop-signal" in type_name:
        await asyncio.sleep(0.1)
        return {"action": "trial_arming_wait", "type_name": type_name, "details": details}

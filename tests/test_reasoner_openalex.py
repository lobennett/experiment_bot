...
@pytest.mark.asyncio
async def test_verify_doi_returns_false_on_title_mismatch():
    """Honest-citation hardening: a real DOI whose OpenAlex title does NOT match
    the claimed title fails verification — catches the wrong-paper fabrication
    mode (year+author coincidence) that previously passed."""
    fake_response = MagicMock()
    fake_response.status_code = 200
    # DOI resolves to a real paper, year + a shared surname match, but the
    # actual paper is a totally different work than what the citation claims.
    fake_response.json = MagicMock(return_value={
        "title": "AIC model selection using Akaike weights",
        "publication_year": 2004,
        "authorships": [{"author": {"display_name": "Eric-Jan Wagenmakers"}}],
    })
    fake_client = MagicMock()
    fake_client.__aenter__ = AsyncMock(return_value=fake_client)
    fake_client.__aexit__ = AsyncMock(return_value=None)
    ...
    with patch("httpx.AsyncClient", return_value=fake_client):
        ok, meta = await verify_doi(
            ...
            expected_authors="Wagenmakers, E.",
            expected_year=2004,
            expected_title="Estimation and interpretation of 1/f noise in choice reaction time",
        )
    assert ok is False, "wrong-paper DOI must fail the title check"


@pytest.mark.asyncio
async def test_verify_doi_title_match_passes():
    """A correct title (token overlap above threshold) still verifies."""
    fake_response = MagicMock()
    fake_response.status_code = 200
    fake_response.json = MagicMock(return_value={
        "title": "Effective analysis of reaction time data",
    ...
    fake_client.get = AsyncMock(return_value=fake_response)
    with patch("httpx.AsyncClient", return_value=fake_client):
        ok, _ = await verify_doi(
            doi="10.1016/j.cognition.2008.07.011",
            expected_authors="Whelan, R.", expected_year=2008,
            expected_title="Effective analysis of reaction-time data",
        )
    assert ok is True

...
**Sequence-response tasks** (the participant reproduces an ordered series of actions within one trial — e.g. clicking several on-screen elements in a required order): in addition to `response_elements`, expose the trial's target order via `correct_sequence_js` — a JS expression that returns the ordered list of target element indices (0-based into `response_elements`) or their visible labels, read from the experiment's runtime state. Put it either on the stimulus's `response` block or, if one expression covers the whole task, in `task_specific.correct_sequence_js`:
...
"response": {
  ...
  "correct_sequence_js": "<JS expression returning the ordered target indices or labels>"
}
```

...
- `cdp_dwell_ms` (default 200.0): Bot dwell in ms before firing each response keypress. The 200 ms default is human-reaction-time territory and fits inside typical response windows (≥1000 ms for Stroop, Flanker, n-back). For stop-signal STOP trials with short SSDs (~250 ms initial), set a lower value (e.g., 100 ms) so go-trial fires remain inside the response window. Override per paradigm class when the response window or SSD warrants.

...
- **Expert-arm references.** The assets the pre-registration cites —
  ...
  collection script,
  and the expert pipeline/dataset — are archived at the
  `expert-arm-final` git tag (main's tip before the naive system was
  merged; `check_doc_links.py` exempts the frozen file for this reason).

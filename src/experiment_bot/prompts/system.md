...
A common structure: the task presents the to-be-reproduced items ONE AT A TIME before the reproduction phase, recording each presented item's position/identifier in its OWN per-item trial-data row (look for the trial that renders a single item and the field its `on_finish` writes the position to). When no live variable holds the full sequence, reconstruct it by reading the last N such per-item rows from the platform's recorded data, in presentation order (e.g. filter the recorded rows to the single-item presentation trials, take the final N, and map each to its recorded position field). Verify against the source which trial id marks those presentation trials and which data field holds the position. Wrap the whole expression so it returns `null` on error rather than throwing. Put it on the stimulus's `response` block, or in `task_specific.correct_sequence_js` if one expression covers the whole task:

// SOURCE: https://github.com/jspsych/jsPsych/blob/jspsych@7.3.1/packages/jspsych/src/modules/plugin-api/KeyboardListenerAPI.ts
// TAG: jspsych@7.3.1
// LICENSE: MIT (see vendor/LICENSES.md)
// RETRIEVED: 2026-05-17
//
// This file is a verbatim copy of the upstream source, vendored as a
// version-pinned API reference for src/experiment_bot/drivers/jspsych/.
// The JsPsychDriver references this file to understand how jsPsych 7.3.1
// dispatches keyboard responses and which methods on pluginAPI it can hook.
//
// Key callouts for the driver:
// - getKeyboardResponse({callback_function, valid_responses, persist, ...})
//   registers a listener on root keydown/keyup. The callback fires with
//   {key, rt}. The driver's monkey-patch captures the callback_function
//   and invokes it directly with (key, rt) for response delivery.
// - rootKeydownListener / rootKeyupListener are attached to
//   getRootElement() in bubble phase (no `true` capture flag).
// - heldKeys tracks keys whose keydown hasn't seen a corresponding keyup;
//   isResponseValid rejects responses for held keys unless allow_held_key
//   is true.
// - case-insensitivity is handled via toLowerCaseIfInsensitive based on
//   areResponsesCaseSensitive (default false → lowercase compare).
